...
async function checkVipStatus() {
  ...
  try {
    ...
    if (profileResult.error) {
      ...
      vipBanner.style.display = "none";
      return { isVip: false, userId: session.user.id };
    }

    if (profileResult.data && profileResult.data.est_vip) {
      vipBanner.style.display = "block";
      return { isVip: true, userId: session.user.id };
    }
  } catch (err) {
...
async function checkCoupons(userId) {
  const access = { htFt: false, scoreExact: false };
  if (!userId) return access;

  try {
    const nowIso = new Date().toISOString();
    const result = await supabase
      .from("abonnements_coupons")
      .select("type_coupon")
      .eq("user_id", userId)
      .eq("actif", true)
      .gte("date_fin", nowIso);

    if (result.error || !result.data) return access;

    result.data.forEach((row) => {
      if (row.type_coupon === "ht_ft") access.htFt = true;
      if (row.type_coupon === "score_exact") access.scoreExact = true;
      if (row.type_coupon === "vip_combine") {
        access.htFt = true;
        access.scoreExact = true;
      }
    });
  } catch (err) {
...
function buildCouponBlock(label, value, unlocked) {
  if (unlocked) {
    return '<div class="coupon-line unlocked"><span class="coupon-label">' + label + '</span><span class="coupon-value">' + (value || "-") + '</span></div>';
  }
  return '<div class="coupon-line locked"><span class="coupon-label">' + label + '</span><span class="coupon-value locked-value">Reserve coupon</span></div>';
}

async function displayMatches(isVip, couponAccess) {
    ...
    if (ticketBody) {
      ticketBody.innerHTML =
        ...
        '<div class="coupon-extras">' +
          buildCouponBlock("HT-FT", hero.htftTip, couponAccess.htFt) +
          buildCouponBlock("Score exact", hero.exactScoreTip, couponAccess.scoreExact) +
        '</div>';
    }
  }
  ...
  if (picksGrid) {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      html += '<div class="pick-card ' + lockedClass + '">' +
        ...
        '<div class="coupon-extras">' +
          buildCouponBlock("HT-FT", match.htftTip, couponAccess.htFt) +
          buildCouponBlock("Score exact", match.exactScoreTip, couponAccess.scoreExact) +
        '</div>' +
        lockOverlay +
        '</div>';
    }
...
async function init() {
  ...
  let userId = null;
  try {
    const vipResult = await checkVipStatus();
    isVip = vipResult.isVip;
    userId = vipResult.userId;
  } catch (err) {
    console.error("Erreur checkVipStatus:", err);
  }

  let couponAccess = { htFt: false, scoreExact: false };
  try {
    couponAccess = await checkCoupons(userId);
  } catch (err) {
    console.error("Erreur checkCoupons:", err);
  }

  try {
    await displayMatches(isVip, couponAccess);
  } catch (err) {
